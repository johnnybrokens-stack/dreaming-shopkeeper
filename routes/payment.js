...
router.get('/pricing', (req, res) => {
  res.json({
    ...
    free: {
      ...
      credits: 5,
      maxWords: 500,
      features: ['1 základní šablona', 'Komunitní podpora']
    }
  });
});
  ...
  try {
    ...
    if (!priceId) {
      return res.status(500).json({
        ...
        message: 'Kontaktujte podporu pro aktivaci předplatného'
      });
    }
    ...
    res.status(500).json({ error: 'Nepodařilo se vytvořit relaci checkout' });
  }
});
...
router.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ error: `Webhook chyba: ${err.message}` });
  }
});
